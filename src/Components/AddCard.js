...
function AddCard() {
    ...
    const onFormSubmit = (Data) =>{
              ...
              axios.post("https://travellingagencywebsite.herokuapp.com/product-api/create-card", formData)
              .then((response)=>{
                  alert((response.data.message));
              })
              .catch((error)=>{
                  console.log(error)
                  alert("Something went wrong in creating Card")
              })
      }
  return (
      <div className='body1 container-fluid'>
          <Form className="w-50 mx-auto p-3 m-3" onSubmit={handleSubmit(onFormSubmit)}>
              <h1 className='Heading display-4  formtitle text-center'>Add New Card</h1>
              <Form.Group className="mb-3" controlId="formBasicEmail">
    <Form.Control type="text" placeholder="Enter Id Number" {...register("IdNo",{required: true})}/>
    {errors.IdNo?.type==='required' && <p className='text-danger text-start '>*This field is required</p>}
  </Form.Group>
  <Form.Group className="mb-3" controlId="formBasicEmail">
